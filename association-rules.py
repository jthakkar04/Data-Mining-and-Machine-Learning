...
class rule:
    ...
    def printRule(self):
        print(self.X, '-->', self.Y, 'sup: ', self.getSupport(), ' conf:', self.getConfidence(), 'lift: ', self.lift,
              'conviction: ', self.conviction)
        fpRules = open("./rules.rul", "a+")
        fpRules.write(str(self.X) + '-->' + str(self.Y) + 'sup: ' + str(self.getSupport()) + ' conf:' + str(
            self.getConfidence()) + 'lift: ' + str(self.lift) + 'conviction: ' + str(self.conviction) + "\n")
...
class Apryori:
    ...
    def combinaItens(self, elem, combine_list):
        combinados = []
        print('To Combine')
        print(elem)
        print('com')
    ...
    def apriori(self, dataset, minsup=None, minconf=None):
        ...
        while (self.listadeCandidatos[i]):
            ...
            print('padrões: ')
            # if(max_index(self.listadePadroes[i][1].items) >= 8):
